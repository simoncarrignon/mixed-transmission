#This is exactly the same script than for Figure4  but withh rho=0.5
...
#We finally re-arranged the original order of the columns, and ultimately remove manullay the duplicate. here with thus did two file, and we manually move the content of the list within allresults_types to make f0.05 become f0, and one duplicate column, the one storing f0, which no mixes the two types (and is removed)
...
for(ctype in 1:2){
	cairo_pdf(paste0("Figure4_rho0.5_Type",gsub("[ &]+","_",renametypes[ctype]),".pdf"),width=12,height=10,pointsize=15)
    ...
    #we use joyplotA, alternative version of joyplot, slighlty adapted for the final version of the figure 2
