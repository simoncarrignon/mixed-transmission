...
pathwaysnames=c(
           ...
           "c_7","c_{8,pt=0.9}","c_8","c_{9,pt=0.9}","c_9")
...
boxplot(at=positions,limited,col=sexcols[as.character(pathways$s[traitsel])],outline=F,type="n",las=2,lwd=.3,ylim=c(0,1),xaxt="n",add=T,boxwex=.80,staplewex=.3,lty=1,ylab="")
...
text(positions,y=par("usr")[3]-0.01,labels=TeX(lapply(colnames(limited),function(i)paste("$",i,"$"))),srt=45,cex=.9,adj=1)
