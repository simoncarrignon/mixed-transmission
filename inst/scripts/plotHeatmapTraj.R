...
#    for(bonus in as.character(c(0,1,3))){
        for(type in 1:2){
            datas=uu[[beta]][[bonus]][,type][[trait]][,350:1]
            if(logfreq)datas=apply(datas,2,function(i){new=log((i+1)/sum(i+1));new-log(1/sum(i+1))})
            #datas=apply(datas,2,function(i)(log(i+1)))
            image(datas,ylab=paste0("c",trait-10),xaxt="n",main=paste("Type",as.roman(i)),col=colorRampPalette(c("white",allsexcols[trait],"black"))(100)[1:70],yaxt="n")
...
axis(1)
axis(4,labels=round(seq(1,200,length.out=5)),at=seq(0,1,length.out=5))
mtext("time step",4,3)
dev.off()
}
