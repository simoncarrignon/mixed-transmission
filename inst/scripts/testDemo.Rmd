...
initcomus$occupation[,]=1
communities=unlist(lapply(1:K,function(i)rep(i,initcomus$size[i])))
population=cbind(newpop(K*50,age="random",community = communities),(K*50,2))
...
pos=random2Dgrid(K=K,Gx=100)
a=initAdaptiveTraits(ki=ki,km=km,n=1 )
initcomus=initialiseCommunities(ki=2,km=0,traits=a,coordinates=pos,G=100,plot=T)
initcomus$size=unname(table(population[,"community"]))
library(parallel)
bs=round(runif(200,.3,.44),digit=3)
cl<-makeCluster(10,type="FORK",outfile="log.txt")
allpopsizesonly=parSapply(cl,bs,function(b){
           modelVector(K=K, m=1, b=b, r=0, rho=.5, d=0.04, maturity=18, endrepro=65, population=population, comus=initcomus, tstep=1000, tp=generatePathways(z=2),age.threshold=20, out=c("popsize"),logging=c("done"),ma=1,traitsid=traitsid,F_Th=100,testdebug=F)$popsize
})
stopCluster(cl)
saveRDS(file="pposizeLargerTh.RDS",allpopsizesonly)
par(mfrow=c(1,3))
boxplot(lengths(allpopsiz)~bs,ylab="simu length vs birth rate ",xlab="birth rate",main="extinctions")
finished=allpopsiz[lengths(allpopsiz)==500]
finsize=sapply(finished,"[",500)
...
plot(allpopsiz[[1]],xlim=c(0,500),ylim=c(0,800))
compressedts=sapply(1:500,function(t)sapply(allpopsiz,function(i)i[t]))
getdist=apply(compressedts,2,quantile)
