...
beta=-1
bonus=runif(100,0.0005,0.008)
beta=runif(100,-1,0.2)
cl<-makeCluster(10,type="FORK",outfile=file.path(expname,"log.txt"))
allpopsizesonly=parSapply(cl,1:100,function(b){
                              set.seed(as.numeric(Sys.time())+b)
                              tryCatch({
                                  a=Sys.time()
                                  singlesimu=modelVector(K=K, m=1, b=0.216, r=bonus[b], rho=1, d=mortality, maturity=18, endrepro=65, population=population, comus=initcomus, tstep=500, tp=fullpathways,age.threshold=20, out=c("popsize"),logging=c("time"),ma=1,traitsid=traitsid,F_Th=100,testdebug=F,fracfiss=.5,beta=beta[b])
                                  print(Sys.time()-a)
