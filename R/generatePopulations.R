...
generateInitialPopulation <- function(km,ki,percomu,z,G,na,F_Th){
    ...
    population=cbind(newpop(K*percomu,age="random",community = communities),generateTraitsMatrix(K*percomu,z))
    newtestres=modelVector(K=K, m=1, b=0.25, r=0, rho=.5, d=c(0.15,0.01,0.01,0.02,0.05,1), maturity=18, endrepro=65, population=population, comus=initcomus, tstep=500, tp=generatePathways(z=z),age.threshold=20, out=c("finalpop"),logging=c("time"),ma=1,traitsid=paste0("t",1:z),F_Th = F_Th)
