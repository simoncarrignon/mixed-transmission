                        ...
                        replicate(5,
                                  {
                                      ...
                                      N=200
                                      population=cbind(newpop(N,age="random",community = communities),initNeutralTraits(N,z))
                                      quickV=modelVector(K=K, m=1, b=0, r=0, rho=1, d=0, maturity=sample(100,1), endrepro=sample(100,1), population=population, comus=initcomus, tstep=sample(z:150,1), tp=neutraltraitsParam,age.threshold=sample(100,1), out=c("finalpop"),logging="done",ma=1,traitsid=paste0("t",1:z))
                                      paste0("t",1:z)
                                      testthat::expect_false(all(apply(quickV$population[,traitsid],2,sum)== apply(population[,traitsid],2,sum) ))
                                      testthat::expect_true(nrow(population)== nrow(population))
                                  })
