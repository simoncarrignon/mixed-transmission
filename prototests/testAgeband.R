...
which(getAgeBand(age.ref=10,age.peers = population.ages,threshold=1,pos="h")) == c(9,10,11)

sapply(1:100,function(i) population.ages[getAgeBand(age.ref=i,age.peers = population.ages,threshold=1,pos="h")] == (i-1):(i+1))

sapply(2:99,function(i) testthat::expect_identical(population.ages[getAgeBand(age.ref=i,age.peers = population.ages,threshold=2,pos="h")], (i-2):(i+2)))
...
testthat::test_that("test oblique age band",{
                        replicate(1000,{
                                      population.ages=sort(rep(population.ages,3))
                                      size.ref=sample(2:100,1)
                                      testthat::expect_identical(dim(getAgeBand(age.ref=sample(1000,size.ref,replace=T),age.peers = population.ages,threshold=10,pos="o")),c(length(population.ages),size.ref))
})
          })
...
testthat::test_that("test oblique age band",{
                        replicate(1000,{
                                      ...
                                      testthat::expect_identical(dim(getAgeBand(age.ref=sample(49,size.ref,replace=T),age.peers = population.ages,threshold=10,pos="o")),c(length(population.ages),size.ref) )
})
          })

size.ref=sample(2:100,1)
population.ages=61:100
testthat::expect_true(all(replicate(100,getAgeBand(age.ref=sample(49,size.ref,replace=T),age.peers = population.ages,threshold=10,pos="o"))))

population.ages=1:60
replicate(100,
          {
              size.ref=sample(2:100,1)
              testthat::expect_true(all(!getAgeBand(age.ref=sample(70:100,size.ref,replace=T),age.peers = population.ages,threshold=10,pos="o")))
          }
)
